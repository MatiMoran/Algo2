...
/* 
    ...
    - |nombresPartidos| >= 2 y nombresPartidos[|nombresPartidos|-1] = "Blanco"
    ...
    - diputadosPorDistrito[X] >= 1 para todo 0 <= X < |diputadosPorDistrito|
    - |diputadosPorDistrito| = |nombresDistrictos|

    - ultimasMesasDistritos[0] >= 1 y ultimasMesasDistritos[X] >= ultimasMesasDistritos[X-1]
      para todo 1 <= X < |ultimasMesasDistritos|
    - |ultimasMesasDistritos| = |nombresDistrictos|
    - No puede haber dos posiciones de "ultimasMesasDistritos" con el mismo valor

    - votosPresidenciales[X] >= 0 para todo 0 <= X < diputadosPorDistrito|
    - |votosPresidenciales| = |nombresPartidos| 

    - |votosDiputadosPorDistricto| = |nombresDistrictos|
    - |votosDiputadosPorDistricto[x]| = |nombresPartidos| para todo 0 <= X < |votosDiputadosPorDistricto|
    - votosDiputadosPorDistricto[x][y] >= 0 para todo 0 <= X < |votosDiputadosPorDistricto| y 0 <= y < |votosDiputadosPorDistricto[0]|
    
    - |HeapsDiputadosPorDistricto| = |nombresDistrictos|
    - Cada elemento de HeapsDiputadosPorDistricto cumple la invariante de MaxHeap
    - |HeapsDiputadosPorDistricto[x]| = |nombresPartidos| para todo 0 <= X < |HeapsDiputadosPorDistricto|
    - Para cada subposicion(X) de cada posicion(Y) de HeapsDiputadosPorDistricto, X.votos = votosDiputadosPorDistricto[Y][X.indicePartido]
    - |mesasRegistradas| = ultimasMesasDistritos[|this.ultimasMesasDistritos| - 1]
    */
